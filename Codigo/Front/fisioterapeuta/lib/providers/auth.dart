...
/*
  Future<void> _authenticateCadastro(String nome, String email, String senha,
      String confirmacao, String urlSegment) async {
    print('entrou no auth');
    final url = 'https://fisioterapiaapp.azurewebsites.net/Usuario/register';
    //final url ='https://flutter-testereq.firebaseio.com/'
...
*/
/*
  Future<void> _authenticateFirebase(
      String email, String senha, String urlSegment) async {
    print('entrou no auth');
    final url =
...
*/
  Future<void> signup(
    ...
    //return _authenticateFirebase(email, senha, 'signUp');
    print('entrou');
    print('nome $nome email $email senha $senha confirmacao $confirmacao ');
    final url = 'https://fisioterapiaapp.azurewebsites.net/Usuario/register';
    //final url ='https://flutter-testereq.firebaseio.com/'
    var response = await http.post(
      url,
    ...
    if (responseBody['error'] != null) {
      ...
      print('body $responseBody'); // imprime oque esta no response
      //_token = responseBody['idToken'];
      //_userId = responseBody['localId'];
      /*_expiryDate = DateTime.now().add(
        Duration(
  ...
  Future<void> login(String email, String senha) async {
    ...
    if (responseBody['message'] != null) {
      ...
      print('token fora : ${responseBody['jwtToken']}');
      print('id fora : ${responseBody['id']}');
      print('corpo : $responseBody');
      _nomeUsuario = responseBody['nome'];
      _userId = responseBody['id'].toString();
      _token = responseBody['jwtToken'];
      _codVinculo = responseBody['codvinculo'];
      print('valor do token : $_token');
      //print(' dentro ${responseBody['codVinculo']}');
      //print('valor do codigo de vinculo : ${responseBody['']}');
      /* _expiryDate = DateTime.now().add(
        Duration(
          seconds: int.parse(
              responseBody['expiresIn']), // a soma da a data de expiracao
